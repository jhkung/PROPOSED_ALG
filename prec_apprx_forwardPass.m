...
for layer_idx = 1:(N_layer-1)
    ...
    for prec_mode_id = 1:length(prec_arr(:,1))
        qW{layer_idx} = qW{layer_idx} + approx_quantMatrix(synapse{layer_idx}.w, prec_mat{layer_idx}.prec_mode{prec_mode_id}, apprx_mat{layer_idx}, prec_arr(prec_mode_id,:));             % quantize the weight{layer_idx}
    end
    ...
    for idx = 1:hiddenSize(layer_idx)
        ...
        for prec_mode_id = 1:length(prec_arr(:,1))
            qA{layer_idx} = qA{layer_idx} + approx_quantMatrix(a{layer_idx}, repmat(prec_mat{layer_idx}.prec_mode{prec_mode_id}(idx,:)',1,num_training), repmat(apprx_mat{layer_idx}(idx,:)',1,num_training) ,prec_arr(prec_mode_id,:));
        end
...
for prec_mode_id = 1:length(prec_arr(:,1))    
    qW{N_layer} = qW{N_layer} + approx_quantMatrix(synapse{N_layer}.w, prec_mat{N_layer}.prec_mode{prec_mode_id}, apprx_mat{N_layer}, prec_arr(prec_mode_id,:));             % Quantize the weight{1}   
end
...
for idx = 1:outputSize
    ...
    for prec_mode_id = 1:length(prec_arr(:,1))
        ...
        curr_prec_mat = prec_mat{N_layer}.prec_mode{prec_mode_id}(idx,:)'; 
        
        qA{N_layer} = qA{N_layer} + approx_quantMatrix(a{N_layer}, repmat(curr_prec_mat,1,num_training),repmat(apprx_mat{N_layer}(idx,:)',1,num_training) ,prec_arr(prec_mode_id,:));
    end
